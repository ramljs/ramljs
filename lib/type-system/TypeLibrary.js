...
class TypeLibrary {
  ...
  addAll(defs) {
    ...
    const orgGet = this.get;
    const creating = {};
    try {
      this.get = (n) => {
        let t = orgGet.call(this, n, true);
        if (!t) {
          const fn = creating[n];
          if (fn) {
            delete creating[n];
  ...
  get(nameOrDef, silent) {
    ...
    nameOrDef = parseTypeName(nameOrDef);
  ...
  create(def) {
    ...
    try {
      ...
      const t2 = this.get(t);
      return new BuiltinTypes[t2.storedType](this, def);
